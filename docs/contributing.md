...
The number of files in the root directory is deliberately kept to a minimum. This is achieved by
moving all configuration files to the `support/root` folder. Whenever you run `pnpm install` these
files are symlinked to the root directory.
...
- `support` - This is the package that provides the configuration files, the website, and many other
  ...
  [look](https://github.com/svgmoji/svgmoji/tree/HEAD/support).

## Build

To build the TypeScript code (which is placed into the dist folders of the respective packages) run
`pnpm build`.
