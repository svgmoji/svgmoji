...
import { isPlainObject, isString, object, omitUndefined } from '@remirror/core-helpers';
  ...
  async function writePackageTsconfig(pkg: Package) {
    ...
    for (const dependency of Object.keys(pkg.dependencies ?? {})) {
      ...
      references.push({
        ...
        path: path.join(path.relative(pkg.location, dependencyPath), tsconfigFileName),
      });
    }
